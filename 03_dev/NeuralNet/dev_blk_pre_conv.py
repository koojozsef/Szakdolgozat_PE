...
from keras.layers import Input, Dense, regularizers, Flatten, Reshape, Conv2D
...
def get_kernel_data(sequence_count, image_count, kernelsize, input_count, height, width):
    ...
    for seq in images:
        ...
        i = 0
        for im in seq:
            im[2] = fn_mog.apply(im[0])*255
            im[4] = fn_knn.apply(im[0])*255
            if i <= 0:
    ...
    for seq in images[:, learn_phase:, :, :, :]:
        for im in seq:
            for k in range(kernel_i, height - kernelsize, kernelsize):
...
def main():
    ...
    encoded1 = Conv2D(20, 3)(input1)
    encoded2 = Conv2D(20, 3)(encoded1)
    encoded3 = Conv2D(20, 3)(encoded2)

    flatten = Flatten()(encoded3)

    decoded = Dense(1,  activation="relu", activity_regularizer=regularizers.l2(10e-5))(flatten)
    ...
    for seqi in check_result_data:
        ...
        i = 0
        for imi in seqi:
            imi[2] = fn_mog.apply(imi[0])*255
            imi[4] = fn_knn.apply(imi[0])*255
            if i <= 0:
            ...
            res_bin = np.array(res_im).astype(np.uint8)
            res_bin[res_bin > 125] = 255
            res_bin[res_bin < 125] = 0
            cv.imshow("NN B", res_bin)
            cv.imshow("NN", res_im.astype(np.uint8))
            cv.imshow("flow", imi[3])
            cv.imshow("knn", (imi[4]).astype(np.uint8))
            cv.imshow("mog", (imi[2]).astype(np.uint8))
